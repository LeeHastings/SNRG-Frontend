...
public class EditorWindow extends BorderPane {
	...
	/**
	 * Create a new window that edits the model in the given 
	...
	EditorWindow(
			PropertiesEditor model, Stage initStage, String title, 
			boolean enableToolbar, boolean enableLayers, 
			boolean enablePathogens, boolean enableEdges)
	{
		this.model = model;
		stage = initStage;
			...
			layerSelect.valueProperty().addListener((o, oldVal, newVal)->{
				...
				if(newVal != null && newVal.isPresent()){
					try {
						...
						String name = this.model.layer_getName(
	...
	private void updateProperties(Optional<Integer> lid) throws EditorException{
		if(lid == null){
			throw new IllegalArgumentException("The layer ID can no longer be null");
		}
		properties.clear();
		if(lid.isPresent()){
